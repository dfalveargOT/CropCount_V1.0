...
class preprocessing:
    ...
    def __init__(self, percentVal, percentFourier, imageRGB):
        ...
        self.image = cv2.bilateralFilter(imageRGB,20,150,150) #20, 150, 150
    ...
    def processingLayer3(self):
        ...
        try:
            _, cnts_01, _ = cv2.findContours((mask_01.copy()).astype('uint8'), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
            _, cnts_02, _ = cv2.findContours((mask_02.copy()).astype('uint8'), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        except:
